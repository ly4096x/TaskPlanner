<script lang="ts">
  ...
  let { task, onclick, minimal = false, draggable = false, ondragstart }: Props = $props();
...
<button class="bg-bg border-none rounded-lg px-2.5 py-2 text-left w-full shadow-[0_1px_2px_rgba(0,0,0,0.05)] transition-[background] duration-150 hover:shadow-[0_2px_6px_rgba(0,0,0,0.15)] hover:brightness-[0.97] {draggable ? 'cursor-grab active:cursor-grabbing' : ''}"
  {draggable}
