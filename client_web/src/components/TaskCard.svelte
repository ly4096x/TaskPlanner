<script lang="ts">
  ...
  let { task, onclick, minimal = false, unread = false }: Props = $props();
...
<div class="bg-bg rounded-lg px-2.5 py-2 text-left w-full overflow-hidden shadow-[0_1px_2px_rgba(0,0,0,0.05)] transition-[background] duration-150 cursor-pointer hover:shadow-[0_2px_6px_rgba(0,0,0,0.15)] hover:brightness-[0.97]"
  ...
  {#if minimal}
    <h3 class="text-sm mb-1 text-text leading-tight break-words {unread ? 'font-bold' : 'font-normal'}">{task.title}</h3>
    <div class="flex items-center justify-between text-sm text-text-secondary opacity-50">
      <span>#{task.id}{#if task.assignee_name}{' '}{task.assignee_name}{/if}</span>
      {#if task.text_comment_count > 0}<span class="inline-flex items-center gap-0.5"><Icon src={ChatBubbleLeft} size="12" />{task.text_comment_count}</span>{/if}
    </div>
  {:else}
