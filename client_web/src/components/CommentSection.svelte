<script lang="ts">
  ...
  let { boardId, taskId }: Props = $props();
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const comment = await addComment(boardId, taskId, text);
      ...
      <div class="flex flex-col gap-2 mb-4">
        {#each sortedComments as comment (comment.id)}
          <div class="bg-bg rounded-[--radius] py-2.5 px-3.5 {comment.comment_type === 'METADATA_CHANGE' ? 'metadata-comment' : ''} {comment.comment_type === 'EXECUTION_LOG' ? 'log-comment' : ''} {comment.comment_type === 'TEXT' ? 'text-comment' : ''}">
            ...
            <div class="prose prose-sm dark:prose-invert max-w-none {comment.comment_type === 'METADATA_CHANGE' ? 'text-xs text-text-secondary' : ''} {comment.comment_type === 'EXECUTION_LOG' ? 'text-[13px] text-text-secondary' : ''} {comment.comment_type === 'TEXT' ? '!text-text' : ''}">
