...
def _enrich_tasks_batch(conn: sqlite3.Connection, tasks: list[dict]) -> list[dict]:
    ...
    comment_rows = conn.execute(
        f"SELECT task_id, COUNT(*) FROM comments "
        f"WHERE task_id IN ({placeholders}) AND comment_type = 'TEXT' "
        f"GROUP BY task_id",
        task_ids,
    ).fetchall()
...
def create_access_token(
    conn: sqlite3.Connection, user_id: int, token_hash: str, label: str = ""
) -> dict:
    created_time = time.time()
    cur = conn.execute(
        "INSERT INTO access_tokens (user_id, token_hash, label, created_time) VALUES (?, ?, ?, ?)",
        (user_id, token_hash, label, created_time),
    )
    conn.commit()
    return {
        "id": cur.lastrowid,
...
def list_access_tokens(conn: sqlite3.Connection, user_id: int) -> list[dict]:
    rows = conn.execute(
        "SELECT id, user_id, label, created_time, last_used_time FROM access_tokens WHERE user_id = ? ORDER BY id",
        (user_id,),
    ).fetchall()
    return [dict(r) for r in rows]


def revoke_access_token(conn: sqlite3.Connection, token_id: int) -> bool:
...
def get_task_access_times(
    conn: sqlite3.Connection, user_id: int, board_id: int
) -> dict[int, float | None]:
    """Get last accessed times for all tasks on a board for a user."""
    rows = conn.execute(
        """
        SELECT tal.task_id, tal.last_accessed_time
        FROM task_access_log tal
        JOIN tasks t ON tal.task_id = t.id
        WHERE tal.user_id = ? AND t.board_id = ?
        """,
        (user_id, board_id),
    ).fetchall()
...
def get_latest_comment_times(
    conn: sqlite3.Connection, board_id: int, comment_type: str = "TEXT"
) -> dict[int, float]:
    """Get latest comment time of given type for each task on a board."""
    rows = conn.execute(
        """
        SELECT c.task_id, MAX(c.created_time)
        FROM comments c
        JOIN tasks t ON c.task_id = t.id
        WHERE t.board_id = ? AND c.comment_type = ?
        GROUP BY c.task_id
        """,
        (board_id, comment_type),
    ).fetchall()
    return {row[0]: row[1] for row in rows}


def record_task_access(
    conn: sqlite3.Connection, user_id: int, task_id: int, accessed_time: float | None
) -> None:
    conn.execute(
        "INSERT INTO task_access_log (user_id, task_id, last_accessed_time) VALUES (?, ?, ?) "
        "ON CONFLICT (user_id, task_id) DO UPDATE SET last_accessed_time = excluded.last_accessed_time",
